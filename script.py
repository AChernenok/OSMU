...
for i in range(number_of_machines):
    ...
    ip_address = "192.168.10." + str(i + 1)
    network_name = "chernenok_vm_" + str(i + 1)
    os.mkdir("./vmid" + str(i + 1))
    vagrantfile_path = "./vm" + str(i + 1) + "/Vagrantfile"
    with open(vagrantfile_path, "w") as file:
        print("\t" + ip_address)
        print("\t" + network_name)
        file.write('Vagrant.configure("2") do |config|\n')
        file.write('\tconfig.vm.define "vm' + str(i + 1) + '" do |vm' + str(i + 1) + '|\n')
        ...
        file.write('\t\t\tvb.name = "' + "chernenokVM" + str(i + 1) + '" \n')
    ...
    print("\t=> Running virtual machine VM" + str(i + 1))
    return_code = subprocess.call(("cd ./vm" + str(i + 1) + " | vagrant up"), shell=True)  
